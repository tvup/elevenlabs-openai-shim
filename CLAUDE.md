...
- **API key resolution**: The `Authorization: Bearer <key>` header is inspected but only used if the key starts with `sk_` (ElevenLabs key format). Keys with other prefixes (e.g. OpenAI `sk-...` keys) are silently ignored, and the server falls back to `XI_API_KEY` from the environment. This ensures compatibility with clients that send OpenAI API keys while still allowing per-request ElevenLabs billing by passing a valid ElevenLabs key.
